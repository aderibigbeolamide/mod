...
export default class RequestCallService {
    repo = dataSource.getRepository(RequestCallEntity);
    ...
    public async createRequestCall(userId: string, createDto: CreateRequestCallDto): Promise<RequestCall> {
        try {
            ...
            requestCall.preferredCallDay = createDto.preferredCallDay;
    ...
    public async getUnprocessedRequestsForHour(): Promise<RequestCall[]> {
        try {
            const now = new Date();
            const oneHourAgo = new Date(now);
            oneHourAgo.setHours(now.getHours() - 1, 0, 0, 0); // previous top of hour
            const thisHour = new Date(now);
            thisHour.setMinutes(0, 0, 0); // current top of hour

            return await this.repo.find({
                where: {
                    processedAt: null,
                    createdAt: Between(oneHourAgo, thisHour)
                },
                relations: ["user", "user.userDetails"],
                order: { createdAt: "ASC" }
            });
        } catch (error) {
            throw error;
